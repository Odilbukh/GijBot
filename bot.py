...
token = "521518490:AAGS1-IVh87IcoYLhiQxuLVihK0YM9UjT1c"
...
def menu(message):
    if message.text == "ℹ️ О нас":
        ...
        d = "https://teletype.in/@gijduvonbot/rk9X-NJIm"
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    ...
    if message.text == "📖 Меню":
        bot.send_photo(message.chat.id, photo=open('test/menu.jpg', 'rb'))
        print(str(message.from_user.first_name), ',', str(message.from_user.last_name), ',', str(message.from_user.id),
              ',',
              '@' + str(message.from_user.username), ',', str(message.from_user.language_code))
    ...
    if message.text == "🛋 Интерьер":
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.add(*[types.KeyboardButton(name) for name in ["Главное меню"]])
        bot.send_photo(message.chat.id, photo=open('test/pic2.jpeg', 'rb'))
        bot.send_photo(message.chat.id, photo=open('test/pic1.jpeg', 'rb'))
        bot.send_photo(message.chat.id, photo=open('test/pic3.jpeg', 'rb'))
        bot.send_photo(message.chat.id, photo=open('test/pic4.jpeg', 'rb'))
        print(str(message.from_user.first_name), ',', str(message.from_user.last_name), ',', str(message.from_user.id),
              ',',
              '@' + str(message.from_user.username), ',', str(message.from_user.language_code))
...
def number(message):
    ...
    message = bot.send_message(message.chat.id, "Очень приятно,  {name}. Оставьте, пожалуйста, Ваш номер телефона: ".format(name=message.text), reply_markup=keyboard)
